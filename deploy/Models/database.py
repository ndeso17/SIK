...
class VehicleIdentity(db.Model):
    ...
    driver_id_card = db.Column(db.String(50), nullable=True)  # Employee ID / KTP
    driver_face_status = db.Column(db.String(20), default='not_available')  # verified, unverified, not_available
    driver_id = db.Column(db.String(50), nullable=True)
    driver_face_image = db.Column(db.String(255), nullable=True)
    driver_confidence = db.Column(db.Float, default=0.0)
    
    # Bounding Box Annotations (JSON stored as Text: [x, y, w, h])
    vehicle_box = db.Column(db.Text, nullable=True)  # Vehicle bounding box
    plate_box = db.Column(db.Text, nullable=True)    # License plate bounding box
    face_box = db.Column(db.Text, nullable=True)     # Driver face bounding box
    body_box = db.Column(db.Text, nullable=True)     # Driver body bounding box
    
    # Active Learning Flag
    is_manually_annotated = db.Column(db.Boolean, default=False)
    ...
    def to_dict(self, include_observations=False):
        result = {
            ...
            'driver_id_card': self.driver_id_card,
            'driver_id': self.driver_id,
            'driver_face_image': self.driver_face_image,
            'driver_confidence': self.driver_confidence,
            'driver_face_status': self.driver_face_status,
            'is_manually_annotated': self.is_manually_annotated,
...
class VehicleObservation(db.Model):
    ...
    # Individual Bounding Box Annotations (JSON stored as Text: [x, y, w, h])
    vehicle_box = db.Column(db.Text, nullable=True)  # Vehicle bounding box
    plate_box = db.Column(db.Text, nullable=True)    # License plate bounding box
    face_box = db.Column(db.Text, nullable=True)     # Driver face bounding box
    body_box = db.Column(db.Text, nullable=True)     # Driver body bounding box
